...
<div class="col-lg-12 grid-margin stretch-card">
    ...
    <tr>
      ...
      <td><a href="/inventory/customer-view/{{$cust->customer_id}}">{{$cust->name}}</a></td>
      <td>{{$cust->deli_note_no}}</td>
      <td>@foreach($cust->salesinvoicedetail as $cuu1)<span style="color: red;">*</span> {{ $cuu1->item_code}} <br/>@endforeach</td>
      <td>@foreach($cust->salesinvoicedetail as $cuu2)<span style="color: red;">*</span> {{ $cuu2->item_name}} <br/>@endforeach</td>
      <td>@foreach($cust->salesinvoicedetail as $cuu)<span style="color: red;">*</span> {{ $cuu->unit}} <br/>@endforeach</td>
      <td>@foreach($cust->salesinvoicedetail as $cuu)<span style="color: red;">*</span> {{ $cuu->quantity}} <br/>@endforeach</td>
      <td>@foreach($cust->salesinvoicedetail as $cuu)<span style="color: red;">*</span> {{ $cuu->isslnrtn_qnty}} <br/>@endforeach</td>
      <td>{{($cust->grand_total)-($cust->isslnrtn_amt)}}</td>
   <td>@if($cust->paidstatus==1) Fully Paid @elseif($cust->paidstatus==2) Partially Paid @else Not Paid @endif</td>
